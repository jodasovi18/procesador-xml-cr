...
class ApiClient:
    ...
    def login(self, usuario: str, clave: str) -> str:
        try:
            ...
            raise ApiError(f"error de conexión en login: {e}") from e
        if r.status_code != 200:
            raise ApiError(f"login falló: HTTP {r.status_code} {r.text[:200]}")
    ...
    def subir_lote(self, token: str, rutas: list[Path]) -> dict:
        ...
        try:
            ...
            raise ApiError(f"error de conexión al subir lote: {e}") from e
        if r.status_code == 401:
            raise NoAutorizado("token inválido o expirado")
        if r.status_code != 200:
            raise ApiError(f"subir_lote falló: HTTP {r.status_code} {r.text[:200]}")
        return r.json()
